 ...
 *
 * Uso: npx ts-node scripts/importar-questoes-enem.ts
 * ================================================================
 */
...
const DISCIPLINA_PARA_AREA: Record<string, { area: string; subarea: string }> = {
...
function escaparSQL(str: string | null | undefined): string {
  if (!str) return ''
  return str
    .replace(/'/g, "''")
    .replace(/\\/g, '\\\\')
...
async function buscarQuestoesAno(ano: number): Promise<QuestaoAPI[]> {
...
function converterQuestao(q: QuestaoAPI): QuestaoSQL | null {
...
function gerarInsertSQL(q: QuestaoSQL): string {
  return `(
    '${escaparSQL(q.id_api)}',
    ${q.ano_prova},
    ${q.numero_questao},
...
async function main() {
  ...
  const todasQuestoes: QuestaoSQL[] = []
...
`
  ...
  const outputPath = path.join(process.cwd(), 'sql', 'questoes_enem_completo.sql')
